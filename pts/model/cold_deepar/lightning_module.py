...
class ColdDeepARLightningModule(pl.LightningModule):
    """
    A ``pl.LightningModule`` class that can be used to train a
    ``ColdDeepARModel`` with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``ColdDeepARModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model
        ``ColdDeepARModel`` to be trained.
    loss
    ...
    def __init__(
        ...
        self.model = ColdDeepARModel(**model_kwargs)
        self.loss = loss
        self.lr = lr
