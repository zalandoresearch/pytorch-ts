...
from .module import LSTNetModel


class LSTNetLightningModule(pl.LightningModule):
    """
    A ``pl.LightningModule`` class that can be used to train a
    ``LSTNetModel`` with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``LSTNetModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model
        ``LSTNetModel`` to be trained.
    loss
    ...
    def __init__(
        ...
        self.model = LSTNetModel(**model_kwargs)
        self.loss = loss
        self.lr = lr
