...
def get_time_window_for_forecast_pts(forward_index, json_data, json_data_train, context_length, forecast_len):
    ...
    if forward_index >= 0:
        for i in range(len(json_data)):
            ...
            json_dynam_feat_test = np.array(json_data[i]['feat_dynamic_real'])[:, :forward_index+forecast_len]
        ...
        for i in range(len(json_data)):            
            ...
            new_json_data[i]['feat_dynamic_real'] = np.array(json_data_train[i]['feat_dynamic_real'])[:, forward_index-context_length:forward_index].tolist()
            new_json_data[i]['start'] = str(past_start_date)
...
def rolling_inference_pts(predictor, train_data, test_data, n_forward):
    ...
    for f in range(n_forward):
        window_instances = get_time_window_for_forecast_pts(f, test_data, train_data, 5, 4)
...
if __name__ == "__main__":
    ...
    predictor = torch.load(f"inference_output/predictor_obj_{special_id}.pt", map_location=torch.device('cpu'))
    ...
    time_window = get_time_window_for_forecast_pts(forward_index, test_data, training_data, context_length, forecast_len)
    
    # time_window_input = ListDataset(time_window, freq = "1D")
    time_window_input = ListDataset(test_data, freq = "1D")
