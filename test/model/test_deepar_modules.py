...
from pts.model.deepar import DeepARLightningModule, DeepARModel
...
def test_deepar_modules(
    ...
    past_time_feat = torch.ones(batch_size, model._past_length, num_feat_dynamic_real)
    future_time_feat = torch.ones(batch_size, prediction_length, num_feat_dynamic_real)
...
def test_rnn_input(prediction_length: int, context_length: int, lags_seq: List[int]):
    num_samples = 10
    ...
    batch = {
        ...
        "past_time_feat": torch.arange(history_length, dtype=torch.float32).view(
            1, history_length, 1
        ),
        "past_target": torch.arange(history_length, dtype=torch.float32).view(
            1, history_length
        ),
        "past_observed_values": torch.arange(history_length, dtype=torch.float32).view(
            1, history_length
        ),
    }
